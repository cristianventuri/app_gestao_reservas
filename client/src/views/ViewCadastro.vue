      ...
      <form class="container-formulario-cadastro" @submit.prevent="this.cadastrar()">
        ...
        <div class="acao-formulario">
          ...
          <Button severity="primary" title="Confirmar" size="small" type="submit">
            <span class="icone-detail">Cadastrar</span>
          </Button>
        </div>
      </form>
    </div>
...
export default {
  ...
  methods: {
    cadastrar() {
      ...
      var confirmaSenha = this.form.confirmaSenha ? this.form.confirmaSenha.trimStart() : '';
      var componenteUser = $("#username");
      var componentePassword = $("#password");
      var componenteConfirmaPassword = $("#confirma_password");
      ...
      if (usuario === '' && senha === '') {
        ...
        this.clearMessages();
        this.mensagem.error = 'Confirme sua senha para prosseguir';
        ...
        Http.post('users', { nome: usuario, senha: senha }).then((resp)=>{
          success('Sucesso', 'Usuário cadastrado com sucesso! Efetue o login para continuar.');
          this.$router.replace('login');
        }).catch((err)=>{
          this.clearMessages();
          const { data } = err.response;
          error('Alerta!', data.message || err.message);
        });
      }
    },
